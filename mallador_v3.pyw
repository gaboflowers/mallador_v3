...
class VentanaMallador(Frame):
    ...
    def nuevo_ramo_inferior(self):
        ...
        self.dibujar_bloques()
        self.creditos_actualizar_cargados()
    ...
    def poblar_widgets_ultimo_ramo(self): #nuevo ramo
        ...
        try:
            uds_ramo = datos_generales["Creditos"]
        except KeyError:
            uds_ramo = "[N/A]"

        Label(frame,text = uds_ramo +" SCT").pack()#side=LEFT)
    ...
    def dibujar_bloques(self):
        ...
        self.creditos_actualizar_mostrados()
    ...
    def status_main(self):
        ...
        self.creditos_cargados = Creditos_Bar(self.frame_ud,u"Créditos Cargados")
        self.creditos_mostrados = Creditos_Bar(self.frame_ud,u"Créditos Mostrados")

        self.choques = ChoquesBar(self.frame_status,"Choques (mostrados)")

    def creditos_actualizar_cargados(self):
        n_uds = 0
        error = False
        for ramo in self.d_cargados.values():
            datos_ramo = ramo[1]
            #uds_ramo = datos_ramo[0]
            uds_ramo = datos_ramo['Creditos'] # <--- cambie datos a dict
        ...
        self.creditos_cargados.set_creditos(n_uds)

    def creditos_actualizar_mostrados(self):
        try:
            n_uds = 0
            error = False
            for cod in self.d_cargados:
            ...
            self.creditos_mostrados.set_creditos(n_uds)
        except AttributeError:
            pass
    ...
    def actualizar_lista_cursos(self):
        ...
        if len(self.union_catalogos[i][1:]) == 0:
            self.lista_cursos.insert(END,u"Nada por aquí")
...
def sct2ud(sct):
    return int(round(5*float(sct)/3))

class Creditos_Bar(StatusBar):
    def __init__(self,frame,texto):
        StatusBar.__init__(self, frame, 25)
        self.text = texto
        self.creditos = "0"
        self.update_label()

    def set_creditos(self, creditos):
        uds_equivalentes = sct2ud(creditos)
        text_uds = " ({} UD)".format(uds_equivalentes)
        self.creditos = creditos+text_uds
        self.update_label()

    def update_label(self):
        self.set(self.text+": "+self.creditos)


class ChoquesBar(StatusBar):
    def __init__(self,frame,texto):
        StatusBar.__init__(self, frame, 18, pk=1)
        self.text = texto
        self.choques = "0"
        self.update_label()
